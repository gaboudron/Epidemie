 ...
 */
public final class Twister extends Random {
  ...
  /**
   * Auto-generated serial version UID. Note that Twister does NOT support serialisation of its
  ...
  private Twister(Mode compatibilityMode) {
    super(0L);
    this.compatibilityMode = compatibilityMode;
  }

  /**
   * Creates a new {@code Twister} instance. This constructor sets the seed of the random number
   ...
   */
  public Twister(boolean compatible) {
   ...
   * that if the same seed data is passed to two different instances of Twister (both of which
  ...
  protected synchronized int next(int bits) {
    ...
    if (mti >= N) { // generate N words at one time
      ...
      // Use the constructor Twister(true) to enable backwards compatible behaviour.
  ...
  private void readObject(ObjectInputStream stream) throws IOException, ClassNotFoundException {
    ...
    if (mt == null || mti < 0 || mti >= mt.length || compatibilityMode == null) {
      throw new IOException("Invalid internal state for Twister");
    }
  }
  ...
  public long genrand_int31() {
    // Implement genrand_int32() using nextLong() and shift the result right by 1 bit
    long randomValue = (genrand_int32() >>> 1);
    // Ensure the value is 31-bit by masking out the 32nd bit
    return (randomValue & 0x7FFFFFFFL);
  }
